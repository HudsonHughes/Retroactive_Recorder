            ...
            audioRecorder.setRecordPositionUpdateListener(new AudioRecord.OnRecordPositionUpdateListener() {
                ...
                public void onPeriodicNotification(AudioRecord recorder) {
                    ...
                    try {
                        ...
                        if(address == null) address = new File(context.getFilesDir() + "/magic/" + String.valueOf(System.currentTimeMillis()) + ".wav");
                        FileOutputStream fos = new FileOutputStream(address, true);
                        fos.write(buffer);
                        fos.close();
                        if(address.length() > HelperClass.rateToByte(HelperClass.getSAMPLERATE()) / 12) address = new File(context.getFilesDir() + "/magic/" + String.valueOf(System.currentTimeMillis()) + ".wav");
                        if(folderSize(new File(context.getFilesDir() + "/magic/")) > HelperClass.getTIME() * HelperClass.rateToByte(HelperClass.getSAMPLERATE()) + HelperClass.rateToByte(HelperClass.getSAMPLERATE()) / 6){
